...
# It verifies the immutable archive SHA256 and stages model files into .local-runtime/models/bilingual-zh-en/
# NOTE: Normal CI ONLY uses this script and NEVER communicates with upstream Hugging Face.
...
# 3. If not found locally, download from Echolet Model Release / Store
if [[ -z "${ARCHIVE_PATH}" ]]; then
    ARCHIVE_PATH="${TMP_WORK_DIR}/${ARCHIVE_NAME}"
    echo "--> Downloading frozen model archive from: ${ARCHIVE_URL}..."
    if ! curl -L --fail --retry 3 --retry-delay 2 -s -o "${ARCHIVE_PATH}" "${ARCHIVE_URL}"; then
        echo "[Warning] Download from ${ARCHIVE_URL} failed."
        echo "--> Falling back to local packaging from staged assets..."
        "${REPO_ROOT}/scripts/package-base-model.sh" "${MODEL_REV}" >/dev/null
